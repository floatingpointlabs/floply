      ...
      <g transform="translate({M.left},{M.top})">
        {#each xTicks as t (t)}
          ...
          <text
            ...
            y={innerH + 18}
            text-anchor="middle"
            class="num fill-[var(--color-ink-faint)] text-[11px]">{fmtTokens(t)}</text
          >
        {/each}
        {#each yTicks as t (t)}
        ...
        {#if optimum && optimum.d > 0 && optimum.n > 0}
          ...
          >
            <title>Budget-optimal: {fmtTokens(Math.trunc(optimum.d))} tokens, {fmtTokens(Math.trunc(optimum.n))}</title>
          </path>
        {/if}
        ...
        <text
          ...
          y={innerH + 36}
          text-anchor="middle"
          class="fill-[var(--color-ink-muted)] text-[12px]">Dataset size (tokens)</text
        >
        <text
          transform="translate({-M.left + 14},{innerH / 2}) rotate(-90)"
          text-anchor="middle"
          class="fill-[var(--color-ink-muted)] text-[12px]">{yAxisTitle}</text
        >
      </g>
    </svg>
