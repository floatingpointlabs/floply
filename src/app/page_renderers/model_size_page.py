...
def render_budget_optimizer_page():
    ...
    with st.expander("Project Setup", expanded=True):
            ...
            if budget_value is not None:
                ...
                _fmt = (
                    ...
                    if _raw >= 1e9
                        ...
                        f"${_raw / 1e6:.2f}M"
                        if _raw >= 1e6
                        else f"${_raw / 1e3:.2f}K" if _raw >= 1e3 else f"${_raw:,.2f}"
                    )
                )
        ...
        if training_type == "Fine-Tuning":
            ...
            if ft_base_model_name and ft_base_model_name != "Custom":
                _sel_model = next(
                    m for m in _known_models if m.name == ft_base_model_name
                )
                ...
                st.caption(
                    "Custom model: parameters estimated using uniform d_model approximation."
                )
            ...
            if ft_method in ("LoRA", "QLoRA"):
                ...
                with lora_col3:
                    ...
                    if (
                        ...
                        _pct = (
                            (_n_adapter / _sel_model.parameter_count * 100)
                            if _sel_model.parameter_count
                            else 0
                        )
                        st.metric(
                            "Adapter Params",
    ...
    target_modules = st.session_state.get(
        "ms_lora_modules", ["q_proj", "k_proj", "v_proj", "o_proj"]
    )
    ...
    if is_ft_with_base:
        ft_base_model = next(
            (m for m in _known_models if m.name == ft_base_model_name), None
        )
        if ft_base_model:
    ...
    budget_display = (
        f"${compute_budget / 1e9:.1f}B"
        if compute_budget >= 1e9
        else (
            f"${compute_budget / 1e6:.1f}M"
            if compute_budget >= 1e6
            else (
                f"${compute_budget / 1e3:.1f}K"
                if compute_budget >= 1e3
    ...
    _opt_flops_n = max(
        int(ft_base_params if (is_ft_with_base and n_flops_base) else n_opt), 1
    )
    ...
    if is_ft_with_base and is_lora and d_opt > 0:
        d_slider_min = max(
            6.0, math.floor(math.log10(max(d_opt, 1e6)) / 0.05) * 0.05 - 2.0
        )
    else:
    ...
    _ft_explore_opts = (
        ...
        if (is_ft_with_base and is_lora)
            ...
            ["Dataset size → token/param ratio"]
            if is_ft_with_base
    ...
    with col_dims:
        ...
        with dm1:
            ...
            if is_ft_with_base and is_lora:
                ...
                _n_help = (
                    f"All {_fmt_tokens(ft_base_params)} params updated during SFT."
                )
            else:
                _n_label = "Model Size"
            ...
            if is_ft_with_base and is_lora:
                ...
                _d_help = (
                    f"Budget-optimal D = {_fmt_tokens(int(d_opt))} for this base model."
                )
            else:
                _d_help = (
        ...
        with dm2:
            st.metric(
                ...
                + (
                    f" Compute uses {_fmt_tokens(ft_base_params)}-param base model FLOPs."
                    if is_ft_with_base and is_lora
                    else ""
                ),
            )
            ...
            st.metric(
                ...
                + (
                    " Adapters only — base weights aren't saved."
                    if is_ft_with_base and is_lora
                    else ""
                ),
            )
    ...
    with col_explorer:
        ...
        if explore_dir == "LoRA Rank → dataset size":
            ...
            st.slider(
                "Model size (params)",
                min_value=n_slider_min,
                max_value=n_slider_max,
                value=n_opt_log_default,
                step=0.05,
                format="10^%.2f",
            ...
            st.slider(
                "Dataset size (tokens)",
                min_value=d_slider_min,
                max_value=d_slider_max,
                value=d_opt_log_default,
                step=0.05,
                format="10^%.2f",
                help=(
                    "Drag to explore dataset sizes. "
                    + (
                        "Compute uses full base model FLOPs; adapter N stays fixed."
                        if is_ft_with_base and is_lora
                        else (
                            "Model size updates to stay within budget."
                            if not is_ft_with_base
    ...
    if is_ft_with_base and is_lora and n_adapter > 0:
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                line=dict(
                    color=CHART_COLORS.get("warning", "#f5a623"), width=1, dash="dot"
                ),
                hovertemplate=(
    ...
    if d_opt >= 10**d_slider_min:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[_star_n],
                name=(
                    "Budget-optimal point" if is_ft_with_base else "Optimal sweet spot"
                ),
                mode="markers",
    ...
    _rec_note = (
        f" (auto: {recommended_instances})"
        if num_instances != recommended_instances
        else ""
    )
    st.info(
